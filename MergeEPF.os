...
Процедура ПроверитьОсновныеПараметрыНаКорректность(Параметры,ИмяФайла,EPFОригинал,EPFИзмененная)
	...
	Если Не ЗначениеЗаполнено(Параметры["СтрокаПодключенияКБазе"]) Тогда
		Стр = ИмяФайла + ". Не указан параметр ""СтрокаПодключенияКБазе""!";
		Лог.Ошибка(Стр);
		ВызватьИсключение(Стр);
	КонецЕсли;	 
	...
	Если НЕ ФайлПроверкаСуществования.Существует() Тогда
		...
		Возврат;
	КонецЕсли;	 
	ФайлПроверкаСуществования = Новый Файл(EPFИзмененная);
	Если НЕ ФайлПроверкаСуществования.Существует() Тогда
		Лог.Ошибка("Файл " + EPFИзмененная + " не существует!");
		Возврат;
	КонецЕсли;	 
КонецПроцедуры
...
Процедура РаспаковатьEPF(ПутьК1С,ИмяФайлаEPF,КаталогРаспаковки,ОсновныеПараметры)
	...
	СтрокаПодключенияКБазе = ОсновныеПараметры["СтрокаПодключенияКБазе"];
	
	
	Команда = ПутьК1С + " " + СтрокаПодключенияКБазе + " /DumpExternalDataProcessorOrReportToFiles """ + КаталогРаспаковки + """ """ + ИмяФайлаEPF + """"; 
...
Процедура ЗапуститьКонфигураторДляБазы(ПутьК1С,КаталогДляБазы)
	Команда = ПутьК1С + " DESIGNER /F" + КаталогДляБазы;
	
	
	retCode = -1;
	ЗапуститьПриложение(Команда,,Истина,retCode);
	Если retCode <> 0 Тогда
		Лог.Ошибка("Конфигуратор вернул после запуска retCode=" + retCode);
		ЗавершитьРаботу(1);
	КонецЕсли;	
КонецПроцедуры
...
Процедура СкопироватьИсходникиОбработкиВКаталогИсходников(КаталогБазы,КаталогИсходниковБазы,КаталогEPF)
...
Процедура ЗагрузитьВБазуКонфигурациюИзКаталога(ПутьК1С,КаталогБазы,КаталогИсходниковБазы)
	
	Команда = ПутьК1С + " DESIGNER /F" + КаталогБазы + " /LoadConfigFromFiles  " + КаталогИсходниковБазы;
	//Сообщить("" + Команда);
...
Процедура ЗагрузитьВБазуEPF(ПутьК1С,КаталогБазы,КаталогEPF,ИмяИзначальногоФайлаEPF)
	...
	КаталогИсходниковБазы = РаспаковатьБазуНаИсходники(ПутьК1С,КаталогБазы);
	
	СкопироватьИсходникиОбработкиВКаталогИсходников(КаталогБазы,КаталогИсходниковБазы,КаталогEPF);
	
	ЗагрузитьВБазуКонфигурациюИзКаталога(ПутьК1С,КаталогБазы,КаталогИсходниковБазы);
...
Процедура СкопироватьСтрокуВБуферОбмена(Стр)
	Команда = "echo " + Стр + " | clip";
	
	ИмяФайлаCMD = ПолучитьИмяВременногоФайла("cmd");
	
	ЗТ = Новый ЗаписьТекста(ИмяФайлаCMD,"UTF-8",,Истина); 
	ЗТ.ЗаписатьСтроку(Команда); 
	ЗТ.Закрыть();
	
	retCode = -1;
	ЗапуститьПриложение(ИмяФайлаCMD,,Истина,retCode);
	Если retCode <> 0 Тогда
		Лог.Ошибка("Не смог скопировать путь к CF в буфер обмена. retCode=" + retCode);
		ЗавершитьРаботу(1);
	КонецЕсли;	
	
	Лог.Информация("Путь к CF для сравнения/объединения находится в буфере обмена.");
...
Функция СобратьEPFИзИсходников(ПутьК1С,ОсновныеПараметры,КаталогОткудаНадоСобиратьEPF,ИмяОбработки)
	ИмяФайлаEPF = ПолучитьИмяВременногоФайла("epf");
	
	СтрокаПодключенияКБазе = ОсновныеПараметры["СтрокаПодключенияКБазе"];
	
	Команда = ПутьК1С + " " + СтрокаПодключенияКБазе + " /LoadExternalDataProcessorOrReportFromFiles  """ + КаталогОткудаНадоСобиратьEPF + "\" + ИмяОбработки + ".xml"" """ + ИмяФайлаEPF + """"; 
	//Сообщить("" + Команда);
	
	retCode = -1;
	ЗапуститьПриложение(Команда,,Истина,retCode);
	Если retCode <> 0 Тогда
		Лог.Ошибка("Не смог собрать <" + ИмяФайлаEPF + "> в <" + КаталогОткудаНадоСобиратьEPF + ">: " + retCode);
		ЗавершитьРаботу(1);
	КонецЕсли;	 
...
Функция ПолучитьEPFИзБазы(ПутьК1С,ОсновныеПараметры,КаталогБазы,ИмяОбработки)
	Лог.Информация("Получение EPF после правки.");
	
	
	КаталогИсходниковБазы = РаспаковатьБазуНаИсходники(ПутьК1С,КаталогБазы);
	КаталогОткудаНадоСобиратьEPF = СоздатьВременныйКаталог();
	
	//КаталогОткудаНадоСобиратьEPF = "c:\temp\777";
	
	ОчиститьКаталог(КаталогОткудаНадоСобиратьEPF);
	
	СкопироватьКаталогСоВсемиФайламииПодкаталогами(КаталогИсходниковБазы + "\DataProcessors",КаталогОткудаНадоСобиратьEPF);
	
	
	ОбрабатьXML_EPF_Из_epf_в_cf(КаталогОткудаНадоСобиратьEPF);
	ИмяФайлаEPF = СобратьEPFИзИсходников(ПутьК1С,ОсновныеПараметры,КаталогОткудаНадоСобиратьEPF,ИмяОбработки);
	
	Возврат ИмяФайлаEPF;
КонецФункции	

Процедура ВыполнитьСкрипт(ОсновнойФайлПараметров,EPFОригинал,EPFИзмененная,EPFMerged)
	...
	ФайлEPFИзмененная = Новый Файл(EPFИзмененная);
	Если НЕ ФайлEPFИзмененная.Существует() Тогда
		Сообщить("Файл " + EPFИзмененная + " не существует!");
		Возврат;
	КонецЕсли;	 
	...
	ПроверитьОсновныеПараметрыНаКорректность(ОсновныеПараметры,ОсновнойФайлПараметров,EPFОригинал,EPFИзмененная);
	...
	КаталогРаспаковкиEPFИзмененная = СоздатьВременныйКаталог();
	Если ОсновныеПараметры["КаталогРаспаковкиEPFИзмененная"] <> Неопределено Тогда
		КаталогРаспаковкиEPFИзмененная = ОсновныеПараметры["КаталогРаспаковкиEPFИзмененная"]; //для отладки
	КонецЕсли;	 
	...
	РаспаковатьEPF(ПутьК1С,EPFИзмененная,КаталогРаспаковкиEPFИзмененная,ОсновныеПараметры);
	
	
	ИмяОбработки = ПолучитьИмяОбработкиИзКаталогEPF(КаталогРаспаковкиEPFОригинал);
	
	
	КаталогДляБазыОригинал   = СоздатьВременнуюБазу(ПутьК1С);
	КаталогДляБазыИзмененная = СоздатьВременнуюБазу(ПутьК1С);
	
	
	ЗагрузитьВБазуEPF(ПутьК1С,КаталогДляБазыОригинал,КаталогРаспаковкиEPFОригинал,EPFОригинал);
	ЗагрузитьВБазуEPF(ПутьК1С,КаталогДляБазыИзмененная,КаталогРаспаковкиEPFИзмененная,EPFИзмененная);
	
	ПутьКИзмененномуCF = ПолучитьCFИзБазы(ПутьК1С,КаталогДляБазыИзмененная);
	
	СкопироватьСтрокуВБуферОбмена(ПутьКИзмененномуCF);
	
	ЗапуститьКонфигураторДляБазы(ПутьК1С,КаталогДляБазыОригинал);
	
	
	ПолученнаяEPF = ПолучитьEPFИзБазы(ПутьК1С,ОсновныеПараметры,КаталогДляБазыОригинал,ИмяОбработки);
	
	//НовыйВариантФайла = СтрЗаменить(EPFОригинал,".epf","-копия.epf");
	НовыйВариантФайла = "./" + EPFMerged;
	
	УдалитьФайлы(НовыйВариантФайла);
	
	КопироватьФайл(ПолученнаяEPF,НовыйВариантФайла);
	Лог.Информация("Обработка закончена.");
	Лог.Информация("Создан файл <" + НовыйВариантФайла + ">");
	
	
КонецПроцедуры

Лог = Логирование.ПолучитьЛог("mergeepf.run.log");
...
Если АргументыКоманднойСтроки.Количество() <> 4 Тогда
	Лог.Ошибка("Не верно переданы параметры! Должно быть: json, epf-original, epf-modif, epf-merged");
	
	Ном = 0;
	Для каждого Аргумент Из АргументыКоманднойСтроки Цикл
		Ном = Ном + 1;
		Сообщить("" + Ном + ". " + Аргумент);
	КонецЦикла;
	
	ЗавершитьРаботу(1);
Иначе	
	ВыполнитьСкрипт(АргументыКоманднойСтроки[0],АргументыКоманднойСтроки[1],АргументыКоманднойСтроки[2],АргументыКоманднойСтроки[3]);
КонецЕсли;
