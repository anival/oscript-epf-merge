...
Процедура РаспаковатьEPF(ПутьК1С,ИмяФайлаEPF,КаталогРаспаковки,ОсновныеПараметры)
	...
	Если СтрокаПодключенияКБазе = Неопределено Тогда
		//СтрокаПодключенияКБазе = СозатьСтрокуПодключенияКБазеДляРаспаковкиEPF();
	КонецЕсли;	 
...
Процедура ОбрабатьXML_EPF(КаталогEPF)
...
Процедура ЗагрузитьВБазуEPF(ПутьК1С,КаталогБазы,КаталогEPF)
	
	СделатьИзмененияВXML_EPF(КаталогEPF);
	ОбрабатьXML_EPF(КаталогEPF);
...
Процедура СкопироватьСтрокуВБуферОбмена(Стр)
	...
	Если retCode <> 0 Тогда
		Лог.Ошибка("Не смог скоипровать путь к CF в буфер обмена. retCode=" + retCode);
		ЗавершитьРаботу(1);
	КонецЕсли;	
КонецПроцедуры

Процедура ВыполнитьСкрипт(ОсновнойФайлПараметров,EPFОригинал,EPFИзмененная)
	...
	ЗагрузитьВБазуEPF(ПутьК1С,КаталогДляБазыОригинал,КаталогРаспаковкиEPFОригинал);
	ЗагрузитьВБазуEPF(ПутьК1С,КаталогДляБазыИзмененная,КаталогРаспаковкиEPFИзмененная);
...
Лог.Информация("Запуск скрипта...");



Если АргументыКоманднойСтроки.Количество() <> 3 Тогда
	Лог.Ошибка("Не верно переданы параметры! json, epf-original, epf-modif");
Иначе	
	ВыполнитьСкрипт(АргументыКоманднойСтроки[0],АргументыКоманднойСтроки[1],АргументыКоманднойСтроки[2]);
КонецЕсли;
